...
export default function OnboardingWizard() {
  ...
  const { user, isOnboarded, completeOnboarding } = useAuth()
  const [step, setStep] = useState(0)

  // Guard: if user is already onboarded, skip to profile immediately
  useEffect(() => {
    if (isOnboarded || user?.isOnboarded) {
    ...
    <div className="min-h-screen bg-cream-100 flex flex-col">
      ...
      <div className="bg-[#F8F3E8] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="font-display font-bold text-lg text-bark-700">AgriConnect</span>
        </div>
