...
import { ArrowLeft, MapPin, Loader2, Info, Star, Tag, ExternalLink } from 'lucide-react'
...
export default function ProductKeywordPage() {
  ...
  useEffect(() => {
    async function fetchByKeyword() {
      ...
      try {
        ...
        // Fetch all listings and find the one matching this keyword slug
        const snapshot = await getDocs(collection(db, 'listings'))
        const allListings = snapshot.docs
          .map(doc => ({ id: doc.id, ...doc.data() }))
          .filter(l => !l.isDraft && l.tab !== 'Wanted')

        // Find listing by keyword match (case-sensitive)
        const match = allListings.find(l => l.keyword === decodedKeyword)

        if (match) {
          setListing(match)
        } else {
          setError('Product not found for this keyword.')
        }
      } catch (e) {
        console.error(e)
  ...
  useEffect(() => {
    if (listing) {
      document.title = `${listing.name} — ${listing.keyword} | CropNGo Marketplace`
      ...
      const tags = [
        'cropngo', 'farm', 'agriculture', 'buy', 'sell', 'malaysia',
        listing.category, listing.name, listing.keyword, ...(listing.tags || [])
      ].filter(Boolean)
      ...
      metaDesc.content = listing.description
        ? listing.description.slice(0, 160)
        : `Buy ${listing.name} on CropNGo Marketplace — ${listing.keyword}.`

      // Open Graph tags for social sharing
      const ogTags = {
        'og:title': `${listing.name} | CropNGo`,
        'og:description': metaDesc.content,
        'og:type': 'product',
        'og:url': window.location.href,
      }
      if (listing.imageUrl) ogTags['og:image'] = listing.imageUrl
  ...
  if (loading) {
    return (
      <div className="bg-[#FAFAF5] min-h-[calc(100dvh-4rem)] flex flex-col items-center justify-center gap-3">
        <Loader2 className="animate-spin text-forest-600" size={36} />
        <p className="text-bark-400 font-body text-sm">Loading product...</p>
      </div>
    )
  }
    ...
    <div className="bg-[#FAFAF5] min-h-[calc(100dvh-4rem)] pb-20 animate-fade-in relative pt-4">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <button 
            onClick={() => navigate(-1)} 
            className="flex items-center gap-2 text-bark-500 hover:text-bark-700 font-body font-bold text-sm w-fit transition-colors"
          >
            <ArrowLeft size={18} />
            Back
          </button>
          ...
          {/* SEO Keyword Badge */}
          <div className="flex items-center gap-2 bg-forest-50 border border-forest-200 text-forest-700 px-4 py-1.5 rounded-full">
            <Tag size={14} />
            <span className="text-sm font-body font-bold">{listing.keyword}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 bg-white border border-cream-200 rounded-[32px] p-6 md:p-8 shadow-sm">
          ...
          <div className="w-full md:w-1/2 flex flex-col pt-2">
            ...
            {listing.tags?.length > 0 && (
              <div className="mb-6 pb-6 border-b border-cream-200/60">
                <h3 className="font-body font-bold text-[#1A1A1A] mb-3 text-sm opacity-60 uppercase tracking-widest">Tags</h3>
                <div className="flex flex-wrap gap-2">
                  {listing.tags.map(tag => (
                    <span key={tag} className="bg-forest-50 border border-forest-200 text-forest-700 text-xs font-bold px-3 py-1.5 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
