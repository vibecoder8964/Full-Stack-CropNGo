...
export default function EventsPage() {
  ...
  const startInitialSearch = () => {
    if (!_bgFetchLoading && user) {
      // Check if we have valid cached results first
      try {
        const cached = localStorage.getItem(CACHE_KEY)
        if (cached) {
          const { timestamp, result } = JSON.parse(cached)
          if (Date.now() - timestamp < CACHE_DURATION) {
            // Use cached results - don't trigger new search
            _bgFetchResult = result
            setHasStartedSearch(true)
            forceUpdate(n => n + 1)
            return
    ...
    try {
      ...
      if (cached) {
        ...
        if (elapsed < CACHE_DURATION) {
          // Cache still valid - use cached results instead of fetching
          console.log('[Events] Using cached results (within 30min)')
          return
        }
      }
    } catch {}

    // Cache expired or not found - run fresh search
    console.log('[Events] Cache expired or not found - running fresh search')
    _bgFetchResult = null
    _bgFetchError = null
    localStorage.removeItem(CACHE_KEY)
    startBackgroundFetch(user, CACHE_KEY)
  ...
  const forceRefresh = useCallback(() => {
    if (!user) return
    // Always run fresh search - bypass cache
    console.log('[Events] Force refresh - running fresh search')
    _bgFetchResult = null
    _bgFetchError = null
    localStorage.removeItem(CACHE_KEY)
    startBackgroundFetch(user, CACHE_KEY)
    ...
    return (
      <div className="bg-[#FAFAF5] min-h-[calc(100dvh-4rem)] pt-8 pb-12">
        <div className="max-w-4xl mx-auto px-4 text-center py-20 bg-white rounded-3xl border border-cream-200 shadow-sm">
          ...
          <button onClick={forceRefresh} className="btn-secondary px-8">Check Again Later</button>
        </div>
      </div>
    )
  }
